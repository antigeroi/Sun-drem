...
class Database:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('''
                CREATE TABLE IF NOT EXISTS characters (
                    ...
                    FOREIGN KEY (cult_id) REFERENCES cults(id) ON DELETE SET NULL,
            ...
            await db.execute('''
                CREATE TABLE IF NOT EXISTS cult_members (
                    cult_id INTEGER,
                    user_id INTEGER,
                    role TEXT DEFAULT 'member' CHECK(role IN ('leader', 'priest', 'member')),
                    joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    PRIMARY KEY (cult_id, user_id),
                    FOREIGN KEY (cult_id) REFERENCES cults(id) ON DELETE CASCADE,
                    FOREIGN KEY (user_id) REFERENCES characters(user_id) ON DELETE CASCADE
                )
            ''')
            ...
            await db.execute('''
                CREATE TABLE IF NOT EXISTS gang_members (
                    gang_id INTEGER,
                    user_id INTEGER,
                    role TEXT DEFAULT 'member' CHECK(role IN ('leader', 'enforcer', 'member')),
                    joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            ...
            # Банки группировок
            await db.execute('''
                CREATE TABLE IF NOT EXISTS group_banks (
                    group_type TEXT CHECK(group_type IN ('guild', 'cult', 'gang')),
                    group_id INTEGER,
                    item_id INTEGER,
                    quantity INTEGER DEFAULT 0 CHECK(quantity >= 0),
                    deposited_by INTEGER,
                    deposited_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    PRIMARY KEY (group_type, group_id, item_id),
                    FOREIGN KEY (item_id) REFERENCES items(id) ON DELETE CASCADE,
                    FOREIGN KEY (deposited_by) REFERENCES characters(user_id) ON DELETE SET NULL
                )
            ''')
    ...
    async def get_character(self, user_id: int) -> Optional[Dict]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            return {
                ...
                'family_id': row[10],
                'last_active': row[11],
                'death_timer_start': row[12],
                'is_alive': bool(row[13]),
                'created_at': row[14],
                'last_search': row[15],
                'active_title': row[16],
                'passport_holder_id': row[17],
                'current_location': row[18],
                'travel_start': row[19],
                'travel_end': row[20],
                'travel_destination': row[21]
            }
    ...
    async def create_cult(self, name: str, description: str, leader_id: int) -> Optional[int]:
        """Создание культа"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                cursor = await db.execute(
                    'INSERT INTO cults (name, description, leader_id) VALUES (?, ?, ?)',
                    (name, description, leader_id)
                )
                cult_id = cursor.lastrowid
                
                await db.execute(
                    'INSERT INTO cult_members (cult_id, user_id, role) VALUES (?, ?, ?)',
                    (cult_id, leader_id, 'leader')
                )
                
                await db.execute(
                    'UPDATE characters SET cult_id = ? WHERE user_id = ?',
                    (cult_id, leader_id)
                )
    ...
    async def create_gang(self, name: str, description: str, leader_id: int) -> Optional[int]:
        """Создание банды"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                cursor = await db.execute(
                ...
                await db.execute(
                    ...
                    (gang_id, leader_id, 'leader')
                )
                
                await db.execute(
                    'UPDATE characters SET gang_id = ? WHERE user_id = ?',
                    (gang_id, leader_id)
                )
    ...
    async def get_cult(self, cult_id: int = None, cult_name: str = None) -> Optional[Dict]:
        """Получение информации о культе"""
        async with aiosqlite.connect(self.db_path) as db:
            if cult_id:
                async with db.execute('SELECT * FROM cults WHERE id = ?', (cult_id,)) as cursor:
                    row = await cursor.fetchone()
            elif cult_name:
                async with db.execute('SELECT * FROM cults WHERE name = ?', (cult_name,)) as cursor:
                    row = await cursor.fetchone()
            else:
                return None
    ...
    async def get_gang(self, gang_id: int = None, gang_name: str = None) -> Optional[Dict]:
        """Получение информации о банде"""
        async with aiosqlite.connect(self.db_path) as db:
            if gang_id:
                async with db.execute('SELECT * FROM gangs WHERE id = ?', (gang_id,)) as cursor:
                    row = await cursor.fetchone()
            elif gang_name:
                async with db.execute('SELECT * FROM gangs WHERE name = ?', (gang_name,)) as cursor:
                    row = await cursor.fetchone()
            else:
                return None
    ...
    async def get_cult_members(self, cult_id: int) -> List[Dict]:
        """Получение участников культа"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                '''SELECT c.user_id, c.character_name, cm.role, cm.joined_at 
                   FROM cult_members cm 
                   JOIN characters c ON cm.user_id = c.user_id 
                   WHERE cm.cult_id = ? AND c.is_alive = TRUE
                   ORDER BY 
                       CASE cm.role 
                           WHEN 'leader' THEN 1
                           WHEN 'priest' THEN 2
    ...
    async def get_gang_members(self, gang_id: int) -> List[Dict]:
        """Получение участников банды"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                '''SELECT c.user_id, c.character_name, gm.role, gm.joined_at 
                   FROM gang_members gm 
                   JOIN characters c ON gm.user_id = c.user_id 
                   WHERE gm.gang_id = ? AND c.is_alive = TRUE
                   ORDER BY 
                       CASE gm.role 
                           WHEN 'leader' THEN 1
    ...
    async def join_cult(self, cult_id: int, user_id: int) -> bool:
        """Вступление в культ"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    'INSERT INTO cult_members (cult_id, user_id) VALUES (?, ?)',
                    (cult_id, user_id)
                )
    ...
    async def join_gang(self, gang_id: int, user_id: int) -> bool:
        """Вступление в банду"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute(
                    'INSERT INTO gang_members (gang_id, user_id) VALUES (?, ?)',
                    (gang_id, user_id)
                )
    ...
    async def leave_cult(self, user_id: int) -> bool:
        """Выход из культа"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                character = await self.get_character(user_id)
                if not character or not character['cult_id']:
                    return False
                
                cult_id = character['cult_id']
                
                cult = await self.get_cult(cult_id=cult_id)
                if cult and cult['leader_id'] == user_id:
                    return False
                
                await db.execute(
                    'DELETE FROM cult_members WHERE cult_id = ? AND user_id = ?',
                    (cult_id, user_id)
                )
                
                await db.execute(
                    'UPDATE characters SET cult_id = NULL WHERE user_id = ?',
                    (user_id,)
                )
    ...
    async def leave_gang(self, user_id: int) -> bool:
        """Выход из банды"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                character = await self.get_character(user_id)
                if not character or not character['gang_id']:
                    return False
                
                gang_id = character['gang_id']
                
                gang = await self.get_gang(gang_id=gang_id)
                if gang and gang['leader_id'] == user_id:
                    return False
                ...
                await db.execute(
                    'UPDATE characters SET gang_id = NULL WHERE user_id = ?',
                    (user_id,)
                )
    ...
    async def deposit_to_group_bank(self, group_type: str, group_id: int, user_id: int, item_id: int, quantity: int) -> bool:
        """Внесение предметов в банк группировки"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                    'SELECT quantity FROM group_banks WHERE group_type = ? AND group_id = ? AND item_id = ?',
                    (group_type, group_id, item_id)
                ) as cursor:
                    existing = await cursor.fetchone()
                
                if existing:
                    await db.execute(
                        'UPDATE group_banks SET quantity = quantity + ? WHERE group_type = ? AND group_id = ? AND item_id = ?',
                        (quantity, group_type, group_id, item_id)
                    )
                else:
                    await db.execute(
                        '''INSERT INTO group_banks (group_type, group_id, item_id, quantity, deposited_by) 
                           VALUES (?, ?, ?, ?, ?)''',
                        (group_type, group_id, item_id, quantity, user_id)
                    )
    ...
    async def withdraw_from_group_bank(self, group_type: str, group_id: int, user_id: int, item_id: int, quantity: int) -> bool:
        """Снятие предметов из банка группировки"""
        try:
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute(
                    'SELECT quantity FROM group_banks WHERE group_type = ? AND group_id = ? AND item_id = ?',
                ...
                if new_quantity <= 0:
                    await db.execute(
                        'DELETE FROM group_banks WHERE group_type = ? AND group_id = ? AND item_id = ?',
                        (group_type, group_id, item_id)
                    )
                else:
                    await db.execute(
                        'UPDATE group_banks SET quantity = ? WHERE group_type = ? AND group_id = ? AND item_id = ?',
                        (new_quantity, group_type, group_id, item_id)
                    )
    ...
    async def get_group_bank(self, group_type: str, group_id: int) -> List[Dict]:
        """Получение содержимого банка группировки"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                '''SELECT gb.item_id, gb.quantity, i.name, i.description, i.type
                   FROM group_banks gb 
                   JOIN items i ON gb.item_id = i.id 
                   WHERE gb.group_type = ? AND gb.group_id = ? AND gb.quantity > 0
                   ORDER BY i.name''',
                (group_type, group_id)
            ) as cursor:
                rows = await cursor.fetchall()
    ...
    async def check_title_conditions(self, user_id: int, title_id: int) -> bool:
        ...
        elif title['condition_type'] == 'cult':
            return character['cult_id'] is not None
        
        elif title['condition_type'] == 'gang':
            return character['gang_id'] is not None
    ...
    async def check_treasury_access(self, treasury_id: int, user_id: int, action: str) -> bool:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Упрощенно: возвращаем True для демонстрации
            return True
